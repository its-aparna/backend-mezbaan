...
export const viewProfile = async (request, response, next) => {
  try {
    ...
    admin = admin?.toObject();
    delete admin?.password;
    admin ? response.status(200).json({ status: true, res: admin }) : response.status(200).json({ status: false });
  } catch (err) {
    return response.status(500).json({ msg: "Internal server error", status: false });
  }
}
...
export const changePassword = async (request, response, next) => {
  try {
    let admin = await Admin.findById(request.body.id);
    if (admin) {
      let saltKey = await bcrypt.genSalt(10);
      let encPass = await bcrypt.hash(request.body.password, saltKey);
...
export const verifyPass = async (request, response) => {
  try {
    const admin = await Admin.findOne({ _id: request.body._id });
    let flag = await bcrypt.compare(request.body.password, admin.password);
    if (flag)
